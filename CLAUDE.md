...
- **Combat hitbox features** (14 floats): `[rel_x, rel_y, w, h, vel_x, vel_y, is_trigger, gives_damage, takes_damage, is_target, is_invincible, anim_progress, hp_raw, hp_max_raw]` plus parallel kind/parent/anim vocab IDs. `gives_damage`=hurts knight on contact, `takes_damage`=has reachable HealthManager (can be attacked), `is_target`=its HealthManager is in `BossSceneController.bosses` (the canonical objective), `is_invincible`=that HealthManager is currently untouchable (post-hit iframes / stagger), without which a staggered boss is indistinguishable from a hittable one and connecting swings look like misses. `vel_x` / `vel_y` are knight-relative per-step displacement computed C#-side against a per-collider position cache — combat rows are emitted from `HashSet` iteration, so row order is unstable between steps and no per-row instance identity crosses the wire, meaning motion is unrecoverable anywhere downstream. `hp_raw` / `hp_max_raw` bypass the running normalizer and get log1p-compressed instead — preserves high resolution near death while keeping the input range bounded (~[0, 8]).
- **Animation channel (the attack telegraph).** Each combat hitbox carries three vocab IDs, embedded and concatenated into its feature row before the combat QFormer: `combat_kind_ids` (what it is), `combat_parent_ids` (whose it is — the owning HealthManager's name), and `combat_anim_ids` (what it is *currently doing* — the clip name off the nearest `tk2dSpriteAnimator`). The first two are cached per collider and never change; the third changes every step, and `anim_progress` is its phase in `[0,1]`, taken as `CurrentFrame / frames.Length` so looping, ping-pong and once-through clips all report sensibly without inspecting `wrapMode`. This is the only anticipatory signal in the observation: a boss's windup clip starts frames before its attack collider spawns, so without it the agent can only react to hitboxes that already exist. Kinds and parents share one vocab and one embedding table (a body's leaf ID and an attack's parent ID are meant to collapse to the same row); clip names get their own vocab (`anim_vocab_size`) and their own table (`anim_embed`), pooled globally rather than namespaced per enemy — "Antic" means much the same everywhere, and the kind/parent embeddings alongside supply the per-boss specialization. Both vocabs are saved in and restored from the PPO checkpoint.
...
- `HitboxObserver`: Tracks all active Collider2Ds via `HitboxReader` MonoBehaviour, classifies into Knight/Enemy/Attack/Terrain. Also owns the per-collider caches that make the time-varying observation columns possible: `prevRelCache` (tick-stamped previous position, differenced for velocity) and `animCache` (nearest `tk2dSpriteAnimator`, for clip name + progress).
- `FsmDumper`: Ground-truth behaviour extraction. HK boss logic is authored as PlayMaker FSMs — serialized data, not compiled code — and the mod runs in-process with `PlayMaker.dll` referenced, so the authored graph is directly readable. Once per scene load it walks every FSM in the boss subtree (plus FSMs on active combat colliders) and serializes states, transitions with their triggering events, every action with its parameters, FSM variables, any-state interrupt transitions, and the `tk2dSpriteAnimator` clip tables. Shipped as a u32-length JSON trailer on the first reset for each scene and written to `python/fsm_dumps/<scene>.json`. Read it with `fsm_graph.py`. This is the answer to anything `fsm_tracker` currently infers: `FsmState.Transitions` is the exact edge set, `SendRandomEvent.weights[]` is the exact branch distribution, `Wait.time` is the exact telegraph duration in seconds, and `Tk2dPlayAnimation.animName` binds a state to the clip name already flowing through `combat_anim_ids`. Action parameters are walked by reflection rather than a per-type switch, so HK's hundreds of custom `FsmStateAction` subclasses all come through; PlayMaker's variable wrappers are unwrapped to either a literal or the name of the variable read, so "always 0.35s" is distinguishable from "reads anticTime" instead of being silently guessed.
- `FsmObserver`: Per-step sampling of `ActiveStateName` only — the runtime *position* in the graph `FsmDumper` describes statically. Debug/visualizer path; never enters the model.
