	...
	public static class SceneHooks
	{
		...
		public static IEnumerator LoadBossScene(string scene_name)
		{
			...
			void Log(string s)
			{
				...
				FullKnight.LogS($"[CanonicalLoad] {s} | scene="
		...
		found:
			...
			for (int attempt = 0; attempt < 3 && panel == null; attempt++)
			{
				...
				FullKnight.LogS($"[CanonicalLoad] pre-SendEvent CONVO START attempt={attempt} state={preSendState}");
				...
				if (sendEx != null)
				{
					FullKnight.LogS($"[CanonicalLoad] SendEvent attempt={attempt} threw {sendEx.GetType().Name}: {sendEx.Message}");
					FullKnight.LogS($"[CanonicalLoad] StackTrace: {sendEx.StackTrace}");
				}
				FullKnight.LogS($"[CanonicalLoad] post-SendEvent attempt={attempt} state={postSendState} finished={postFinished}");

				panel = UnityEngine.Object.FindObjectOfType<BossChallengeUI>();
				if (panel == null)
				{
					FullKnight.LogS($"[CanonicalLoad] attempt={attempt} panel not same-frame; yielding once and retrying find");
					yield return null;
					panel = UnityEngine.Object.FindObjectOfType<BossChallengeUI>();
				}
				if (panel != null) break;
				...
				// and retry.
				FullKnight.LogS($"[CanonicalLoad] attempt={attempt} retry path: panel missing, fsm at {postSendState} finished={postFinished}; reinitializing");
				try
				{
					statue.bossUIControlFSM.Fsm.Reinitialize();
					statue.bossUIControlFSM.Fsm.Start();
				...
				{
					FullKnight.LogS($"[CanonicalLoad] reinit threw {e.GetType().Name}: {e.Message}");
				}
				yield return null;
			}
			if (panel == null)
			{
				...
				FullKnight.LogS($"[CanonicalLoad] panel-missing final-diag "
			...
			FullKnight.LogS($"[CanonicalLoad] pre-LoadBoss state={preLoadBossState}");
		...
		private static IEnumerator LogFsmTransitions(PlayMakerFSM pmFsm, string tag, float timeoutSec)
		{
			...
			FullKnight.LogS($"[CanonicalLoad/{tag}] timeout after {timeoutSec}s, last state='{last}' finished={lastFinished}");
		}
		...
		private static void LogHero(string tag)
		{
			...
			if (hc == null) { FullKnight.LogS($"[CanonicalLoad/hero/{tag}] HeroController.instance=null"); return; }
		...
		private static IEnumerator LeaveBenchIfSitting(float timeoutSec = 30f)
		{
			...
			if (pd == null || hc == null)
			{
				FullKnight.LogS("[LeaveBench] no PD/HC; skipping");
				yield break;
			}
			...
			if (!startedOnBench)
			{
				FullKnight.LogS("[LeaveBench] pd.atBench=false; nothing to leave");
				yield break;
			}
			var shim = InputDeviceShim.Attached;
			if (shim == null)
			{
				FullKnight.LogS("[LeaveBench] no shim attached; cannot tap");
				yield break;
			}

			FullKnight.LogS("[LeaveBench] tapping Jump+Attack until knight stands");
			float t0 = Time.realtimeSinceStartup;
			int frame = 0;
			bool keysDown = false;
			int lastSec = -1;
		...
		private static IEnumerator DriveDreamReturnWakeUp(float timeoutSec = 6f)
		{
			...
			foreach (var s in inventoryStates)
			{
				...
				FullKnight.LogS($"[WakeUp/inv-nonpause] {s}");
			}
			...
			if (hc != null)
			{
				...
				{
					FullKnight.LogS("[WakeUp/heroFsm] 'Dream Return' FSM not found on HC");
				}
			}

			if (driving == null)
			{
				FullKnight.LogS("[WakeUp] no active 'Return from boss' FSM; nothing to wake");
				yield break;
			}
			...
			try
			{
				...
				if (fsm.States != null)
				{
					foreach (var state in fsm.States)
					{
						...
						sb.Append("]");
						FullKnight.LogS(sb.ToString());
					}
				}
			}
			catch (System.Exception e)
			{
				FullKnight.LogS($"[WakeUpFsm/{tag}] dump err: {e.Message}");
			}
		}
		...
		private static void LogHcDuringWake(string doorState, bool doorFinished, int sec, string heroFsmState)
		{
			...
			if (hc == null)
			{
				FullKnight.LogS($"[WakeUp/hc-tick={sec}] HC=null doorState={doorState}");
				return;
			}
		...
		private static void DumpStatueFSM(BossStatue statue)
		{
			...
			{
				FullKnight.LogS("[StatueFSM] no statue or no bossUIControlFSM");
				return;
			}
