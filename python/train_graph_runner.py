...
non-capturable code path. Side benefit: LR annealing via set_lr() still
works (capturable=True would have baked LR at capture time).
...
Why this is safe:
  ...
  - All Categorical distributions in the policy already pass
    validate_args=False (capture rejects host-syncing ops).
  - gru_info["gru_norm"] is a tensor (model.py forward_sequence) so we
    can copy it into the static output buffer instead of .item()ing.
...
Long-moss difference vs main: value-loss normalization uses per-boss
EMA variance, so each chunk in the minibatch has its own (atk_var,
def_var) instead of a per-rollout scalar. The runner exposes static
(CPB,) buffers for these and copies the slice atk_var_t[idx] /
def_var_t[idx] in before each replay.

Caveats:
  - Mirror augmentation MUST happen on the source minibatch tensors
    before they are copied into the static buffers — the mirror op
    itself is not inside the graph.
  - Combat / terrain dims are ceiling-padded to the next available
    bucket and zero-filled; padded slots have mask=0 so attention
    ignores them.
"""
import torch
...
class BucketedTrainGraphRunner:
    ...
    def _make_input_buffers(self, n_combat, n_terrain):
        ...
        return {
            ...
            "valid":             z((CPB, L)),
            "committed":         z((CPB, L)),
            # Per-chunk value-loss variance denominators (long-moss has per-boss
            # EMA variance; all L steps in a chunk share a boss so a (CPB,)
            # tensor is enough — broadcast over L inside the captured step).
            # Seeded with ones so the warmup forward doesn't divide by zero.
            "atk_var":           torch.ones((CPB,), dtype=f32, device=device),
            "def_var":           torch.ones((CPB,), dtype=f32, device=device),
        }
    ...
    def _step_fn(self, d_in, d_out):
        ...
        def step():
            ...
            # Per-sample variance denominator: expand (CPB,) over L.
            atk_var_flat = d_in["atk_var"].unsqueeze(-1).expand(-1, L).reshape(-1)
            def_var_flat = d_in["def_var"].unsqueeze(-1).expand(-1, L).reshape(-1)
    ...
    def run(self, obs_mb, hx_mb, act_mb,
        ...
        # Zero the combat/terrain padding region before copying — masks are
        # also copied, but copy_ on slices leaves the rest with stale data
        # from prior replays, which would corrupt attention's softmax via
        # the kind-embed lookup if combat_kind_ids has stale nonzero ids in
        # pad slots.
        ...
        d_in["atk_var"].copy_(atk_var_mb)
        d_in["def_var"].copy_(def_var_mb)

        slot["graph"].replay()
        # optimizer.step runs EAGER, after the captured backward has
        # populated p.grad. The grad tensor addresses are stable (allocated
        # during warmup with set_to_none=False), so the optimizer reads the
        # values the captured graph just wrote.
        self.optimizer.step()
        return slot["d_out"]
