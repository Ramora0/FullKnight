...
class KindVocab:
    """String -> int vocab. `label` only names the vocab in log output, so a
    run with several vocabs (hitbox kinds, animation clips) says which one grew
    or overflowed."""

    def __init__(self, max_size=512, label="kind"):
        self._s2i = {}
        self._i2s = []
    ...
    def encode(self, s):
        ...
        if len(self._i2s) >= self.max_size:
            if s not in self._overflow_warned:
                ...
                bar = "!" * 78
                print("\n" + bar, flush=True)
                print(f"!!  {self.label.upper()} VOCAB OVERFLOW (cap={self.max_size})", flush=True)
                print(f"!!  Dropping new {self.label}: {s!r}", flush=True)
                print(f"!!  Routing to 'unknown'. Increase config.{self.label}_vocab_size to fix.", flush=True)
                print(f"!!  Currently dropped: {sorted(self._overflow_warned)}", flush=True)
                print(bar + "\n", flush=True)
            return self._s2i[UNKNOWN]
        idx = self._add(s)
        print(f"[vocab] new {self.label} #{idx}: {s!r}", flush=True)
        return idx
    ...
    def load_state_dict(self, state):
        ...
        self.label = state.get("label", self.label)
        self._overflow_warned = set()
