...
def make_rollout_inputs(cfg):
    obs = Observation(
        ...
        combat_anim_ids=torch.zeros(B, N_COMBAT, dtype=torch.long, device=DEVICE),
...
def make_train_inputs(cfg):
    obs = Observation(
        ...
        combat_anim_ids=torch.zeros(B, L, N_COMBAT, dtype=torch.long, device=DEVICE),
...
def bench_rollout_graph(model, obs, hx, dtype):
    ...
    static_obs = Observation(
        ...
        combat_anim_ids=obs.combat_anim_ids.clone(),
