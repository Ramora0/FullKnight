...
def make_obs(nc=20, nt=30, dev=device):
    return Observation(
        ...
        combat_anim_ids=torch.zeros(B, nc, dtype=torch.long, device=dev),
...
np_obs = Observation(
    ...
    combat_anim_ids=obs.combat_anim_ids.cpu().numpy(),
...
def numpy_to_gpu_obs(o):
    return Observation(
        ...
        combat_anim_ids=torch.from_numpy(o.combat_anim_ids).long().to(device),
...
def make_numpy_obs(ci):
    ...
    return Observation(
        ...
        combat_anim_ids=np.zeros((B, nc), dtype=np.int64),
