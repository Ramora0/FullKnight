...
with torch.no_grad():
    for _ in range(WARMUP):
        model.get_action_and_value(combat_hb, combat_mask, terrain_hb, terrain_mask, global_state)
...
try:
    ...
    with torch.no_grad():
        for _ in range(WARMUP):
            compiled.get_action_and_value(combat_hb, combat_mask, terrain_hb, terrain_mask, global_state)
    torch.cuda.synchronize()
